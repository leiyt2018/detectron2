...
class EvidenceLoss(BaseWeightedLoss):
    ...
    def edl_loss(self, func, y, alpha, annealing_coef, target):
       ...
       #
       #  fg_mask = target < 20
       #  fg_id = fg_mask.nonzero(as_tuple=False)
       #  alpha_fg = alpha[fg_id]
       #  gt_classes_fg = target[fg_id]
       #
       #  bg_mask = target >= 20
       #  bg_id = bg_mask.nonzero(as_tuple=False)
       #
       #
       #  S = torch.sum(alpha_fg, dim=1, keepdim=True)
       #  A = torch.sum(y * (func(S) - func(alpha)), dim=1, keepdim=True)
       #
       # # losses.update({'loss_cls': A})
       #  losses.update({'loss_cls':torch.mean(A)})
       #
       #  losses.update({'lambda': annealing_coef})
       #  if self.with_kldiv:
       #      kl_alpha = (alpha - 1) * (1 - y) + 1
       ...
       #      losses.update({'loss_avu': torch.mean(avu_loss)})
        losses = {}
        fg_mask = target < 20
        fg_id = fg_mask.nonzero(as_tuple=False)
        fg_id =fg_id.squeeze(1)
        alpha_fg = alpha[fg_id][:,:-1]
        gt_classes_fg = target[fg_id]
        y_fg = y[fg_id][:,:-1]

        # 前景的证据损失
        S_fg = torch.sum(alpha_fg, dim=1, keepdim=True)
        A = torch.sum(y_fg * (func(S_fg) - func(alpha_fg)), dim=1, keepdim=True)

        # 前景的证据校准
        uncertainty_fg = (self.num_classes-1)/S_fg
        uncertainty_loss_fg = -torch.log(1 - uncertainty_fg + self.eps)
        # loss_func = torch.nn.MSEloss()
        # uncertainty_loss_fg = loss_func(uncertainty_fg, 0)

        # 背景的证据校准
        bg_mask = target >= 20
        bg_id = bg_mask.nonzero(as_tuple=False)
        bg_id = bg_id.squeeze(1)
        alpha_bg = alpha[bg_id][:,:-1]
        if alpha_bg.shape[0]!=0:
            S_bg = torch.sum(alpha_bg, dim=1, keepdim=True)
            uncertainty_bg = (self.num_classes - 1) / S_bg
            # print("alpha_bg:{}".format(alpha_bg))
            # print("S_bg:{}".format(S_bg))
            uncertainty_loss_bg = - torch.log(uncertainty_bg + self.eps)
            avu_loss = torch.mean(uncertainty_loss_bg) + torch.mean(uncertainty_loss_fg)
            # print("uncertainty_bg:{}".format(uncertainty_bg))
            # print("uncertainty_loss_bg:{}".format(uncertainty_loss_bg))
        else:
            avu_loss = torch.mean(uncertainty_loss_fg)

        # print("uncertainty_fg:{}".format(uncertainty_fg))
        # print("uncertainty_loss_fg:{}".format(uncertainty_loss_fg))
        # avu_loss = torch.mean(uncertainty_loss_bg) + torch.mean(uncertainty_loss_fg)
...
if __name__ == "__main__":
    output = torch.tensor([[2, 4, 8], [1, 6, 3]])
    ...
    edl_loss = EvidenceLoss(num_classes=3,
                            evidence='exp',
                            loss_type='log',
                            with_kldiv=False,
                            with_avuloss=True,
                            annealing_method='exp')
    loss = edl_loss(output, label, epoch=20, total_epoch=50)
    print(loss)
