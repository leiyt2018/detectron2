...
# f = open("/home/leiyvtian/datasets/ImageNet1k_Val_OE/coco_instances_results.json", 'r')
# content = f.read()
# a = json.loads(content)
# # annsImgIds_res = [ann['image_id'] for ann in a]
# f.close()
# f = open(ImageNetVal_GT_file, 'r')
# content = f.read()
# a = json.loads(content)
# annsImgIds_gt = [ann['id'] for ann in a['images']]
# f.close()
# annsImageIds_res_set = set(annsImgIds_res)
# annsImgIds_gt_set = set(annsImgIds_gt)
# Evaluate detection results
# with open("/home/leiyvtian/datasets/ImageNet1k_Val_OE/oe_val.txt",'r') as f:
#     lines = f.readlines()
#     image_ids = []
#     for l in lines:
#         image_ids.append(l.strip('\n'))
# f = open("/home/leiyvtian/datasets/ImageNet1k_Val_OE/coco_instances_results.json", 'r')
# content = f.read()
# a = json.loads(content)
# a = [item for item in a if item['image_id'] in image_ids]
# # annsImgIds_res = [ann['image_id'] for ann in a]
# f.close()
# with open(os.path.join('/home/leiyvtian/datasets/ImageNet1k_Val_OE', 'coco_instances_results_rpn_max_proposal_oe.json'), 'w') as fp:
#     json.dump(a, fp, indent=4, separators=(',', ': '))
...
for key, gt_values in gt_coco_api.imgToAnns.items():
    ...
    if len(matched_labels) != 0:
        ...
        if_detected = len(torch.nonzero(torch.where(matched_labels==1,matched_labels,0))) >=2
        if if_detected:
...
plt.title("Max proposal@128 with IOU >0.5")
plt.xlabel("classes")
plt.ylabel("number")
...
# plt.show()
#
# with open("/home/leiyvtian/datasets/ImageNet1k_Val_OE/removed_ids.txt", 'r') as f:
#     _image_level_label = f.readlines()
# image_level_label = []
# for item in _image_level_label:
#     image_level_label.append(item.strip('\n'))
# with open("./stat_max_128proposal.txt",'w') as f:
#     for key, value in freq.items():
#         if key not in image_level_label:
#             f.write("{}:{}\n".format(key,value))
# f.close()
#
