...
def main(args):
    ...
    with open(os.path.expanduser(os.path.join(dataset_dir, 'Annotations', 'classes.csv')), 'r', encoding='utf-8') as f:
        csv_f = csv.reader(f)
    ...
    annotations_csv_path = os.path.expanduser(
        os.path.join(dataset_dir, 'Annotations', 'detections.csv'))
    image_dir = os.path.expanduser(os.path.join(dataset_dir, 'Images_val_v3'))
    ...
    json_dict_val = {'info': {'year': 2022},
                     'licenses': licenses,
                     'categories': categories,
                     'images': images_list,
                     'annotations': annotations_list}

    val_file_name = os.path.join(output_dir, 'val_v3_coco_format.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-dir",
        default='E:\Datasets\AAAI2023\OpenImagesCOCO_Ours_Trainingset_Version\BalancedBenchmark',
        required=False,
        type=str,
