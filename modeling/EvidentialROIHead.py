...
from modeling.EvidentialFastRCNNOutputLayers import EvidentialFastRCNNOutputLayers
...
@ROI_HEADS_REGISTRY.register()
class EvidentialROIHeads(ROIHeads):
    """
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ##新添加的组件
        ret["box_predictor"] = EvidentialFastRCNNOutputLayers(cfg, ShapeSpec(channels=out_channels,height=1,width=1))
    ...
    def forward(
        ...
        if self.training:
            del features
            losses = self.box_predictor.losses(predictions, proposals,epoch=epoch,total_epoch=total_epoch)
            if self.mask_on:
