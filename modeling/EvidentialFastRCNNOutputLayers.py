...
from detectron2.layers import edl_loss
from detectron2.layers import edl_loss_objectdetection
import torch.nn.functional as F

__all__ = ["evidential_fast_rcnn_inference", "EvidentialFastRCNNOutputLayers"]
...
def evidential_fast_rcnn_inference(
    ...
    """
    Call `evidential_fast_rcnn_inference_single_image` for all images.
    ...
    """
    result_per_image = [
        evidential_fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, uncertainty,image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for scores_per_image, boxes_per_image, image_shape,uncertainty in zip(scores, boxes, image_shapes, uncertainty)
    ]
...
def evidential_fast_rcnn_inference_single_image(
    boxes,
    scores,
    uncertainty,
    ...
    # 新添加
    uncertainty = uncertainty[filter_inds[:,0],0]

    # 2. Apply NMS for each class independently.
    keep = batched_nms(boxes, scores, filter_inds[:, 1], nms_thresh)
    if topk_per_image >= 0:
        keep = keep[:topk_per_image]
    boxes, scores, filter_inds, uncertainty = boxes[keep], scores[keep], filter_inds[keep],uncertainty[keep]
...
class EvidentialFastRCNNOutputLayers(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # 新添加的edl_loss
        # self.edl_loss = edl_loss.EvidenceLoss(num_classes=num_classes+1,
        #          evidence='relu',
        #          loss_type='log',
        #          with_kldiv=False,
        #          with_avuloss=True,
        ...
        self.edl_loss = edl_loss_objectdetection.EvidenceLoss(num_classes=num_classes+1,
                 evidence='exp',
                 loss_type='log',
                 with_kldiv=False,
                 with_avuloss=True,
    ...
    def losses(self, predictions, proposals, epoch, total_epoch):
        """
        Args:
        ...
        # print(scores)
        # print(gt_classes)
        loss_cls = self.edl_loss(scores,gt_classes,epoch=epoch,total_epoch=total_epoch)
        # print(loss_cls)
        loss_cls_edl = loss_cls['loss_cls']
        loss_cls_avu = loss_cls['loss_avu']

        losses = {
            "loss_cls_edl": loss_cls_edl,
            "loss_cls_avu": loss_cls_avu,
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        scores, uncertainty = self.predict_probs(predictions, proposals)
        image_shapes = [x.image_size for x in proposals]
        # 有修改
        return evidential_fast_rcnn_inference(
            boxes,
            scores,
            uncertainty,
            image_shapes,
    ...
    def predict_probs(
        ...
        # evidence = torch.exp(torch.clamp(scores,-10, 10))
        # evidence = torch.clamp(scores,min=0,max=None)
        # evidence = F.(scores)
        evidence = torch.exp(torch.clamp(scores[:,:-1], 10, 10))
        alpha = evidence + 1
        S = torch.sum(alpha[:,:-1], dim=1, keepdim=True)
        probs = alpha / S
        uncertainty = 20.0 / S
        return probs.split(num_inst_per_image, dim=0), uncertainty.split(num_inst_per_image, dim=0)
